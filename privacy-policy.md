...
Prompt text, reference images, and generation parameters — sent when you click Generate or Run All.

6.2 Google Gemini (gemini.google.com)
Browser tab navigation only — when you select Gemini as the AI provider.

6.3 User-Configured Third-Party API Endpoints (Optional)
Prompt text and API key — sent directly from your browser to the endpoint URL you configured. The developer does not act as an intermediary and has no access to your keys.

6.4 GitHub (raw.githubusercontent.com)
No user data is sent. The Extension only downloads a configuration JSON file from this address.

6.5 No Other Parties
We do not sell, rent, trade, or share any user data with any other third parties. No advertising, analytics, or tracking services are used.

7. Permissions Justification
Permission	Justification
activeTab	Detect whether the current tab is a Google Labs Flow page
tabs	Communicate with Google Labs Flow and optionally open Gemini tabs
storage	Save settings, projects, task queues, and remote config cache locally
unlimitedStorage	Store large task queue and project history without hitting Chrome's storage limits
scripting	Inject automation scripts into Google Labs Flow to execute generation requests
sidePanel	Display the Extension UI as a Chrome Side Panel
downloads	Download generated images and videos to your device
clipboardRead	Read clipboard content when you explicitly paste into the Extension
Host Permissions
Host	Justification
https://labs.google/*	Communicate with Google Labs Flow
https://gemini.google.com/*	Interact with Google Gemini for prompt generation
https://raw.githubusercontent.com/*	Fetch the UI configuration file for compatibility updates
8. User Rights and Data Control
View: All settings, projects, and task queues are visible within the Extension.
Export: Use "Xuất dự án JSON" to download your project data.
Delete: Delete projects or clear the queue within the Extension, or uninstall to remove all local data.
Revoke: Disable or uninstall the Extension at any time via Chrome's extension management page.
9. Security
Data is stored via Chrome's chrome.storage.local API, sandboxed from other extensions and websites.
...
We may update this policy to reflect changes in functionality or for legal compliance. The "Last updated" date will be revised accordingly.

12. Contact Us
Email: linhbp0808@gmail.com
Discord: https://discord.gg/WH2csC6ut3
